...
public class LessonController {
    ...
    public ResponseEntity<LessonAdminDTO> createLesson(@PathVariable int courseId, @Valid @RequestBody LessonAdminDTO lesson) {
        return new ResponseEntity<>(lessonService.createLesson(courseId, lesson), HttpStatus.CREATED);
    }

    @PutMapping("/lessons/{lessonId}")
    public ResponseEntity<LessonAdminDTO> updateLesson(@PathVariable int lessonId, @Valid @RequestBody LessonAdminDTO lesson) {
